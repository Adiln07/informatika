						...
						<tbody>
							<?php
								...
								<tr>
									...
									<td ><?=($data->gelar_depan != '' ? $data->gelar_depan.'. ' : '').$data->nama_dosen .($data->gelar_belakang != '' ? ', '.$data->gelar_belakang : '')?><br>XXXXXXXX </td>
									<td ><?=$data->jabatan_dosen?></td>
									<td ><?=$data->bidang_penelitian?></td>
									<td ><?=$data->email_dosen?></td>
									<td ><a href="<?=base_url('dosen/').$data->nip?>" class="btn btn-primary gelap">Profil</a></td>
								</tr>
								<?php $i++; } }?>
						</tbody>
					</table>
